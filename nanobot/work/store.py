...
class WorkStore:
    ...
    def __init__(self, workspace: Path):
        ...
        # MIT-1010: the store is constructed for every AgentLoop, including
        # tenants that never schedule Work.  Opening (and therefore creating)
        # the SQLite file eagerly turns an unwritable or not-yet-created
        # workspace into an AgentLoop construction failure, so the schema is
    ...
    def _ensure_ready(self) -> None:
        ...
        self._init_db()
        # Set before the follow-up queries: they go through ``_connect``, which
