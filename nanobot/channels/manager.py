...
class ChannelManager:
    ...
    async def _send_once(channel: BaseChannel, msg: OutboundMessage) -> None:
        ...
        # Ziggy-local (fork): Discord progress heartbeat — one evolving status
        # message per turn, edited in place. Keyed by chat_id only (no stream
        # id) so it survives tool-call segment boundaries. Handled before the
        # upstream event dispatch because it is not an upstream event type.
        if msg.metadata.get("_status_delta"):
            send_status = getattr(channel, "send_status", None)
            if callable(send_status):
                await send_status(msg.chat_id, msg.content, msg.metadata)
            return
