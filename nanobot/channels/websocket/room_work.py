...
def connected_read_executor(agent: Any, server_name: str, mcp_provider: Any = None):
    ...
    async def execute(action: dict[str, Any]) -> str:
        ...
        # 0.3.0 shortens long MCP tool names (upstream ``3f9fb63d``), so the
        # lookup must go through the same helper the wrapper registers with --
        # ``_sanitize_name`` alone no longer reproduces the registered name.
        from nanobot.agent.tools.mcp import MCPToolWrapper, _sanitize_mcp_tool_name
