...
class _ZiggyTurnHook(AgentHook):
    """Ziggy-local (fork): per-turn telemetry + Discord progress heartbeat.

    Re-applied on top of upstream's turn-hook architecture. The pre-upgrade
    fork carried this as ``_LoopHook`` wired into the old callback pipeline
    (``on_progress`` / ``on_stream`` / ``on_status`` positional callbacks);
    upstream replaced that pipeline with typed events plus per-turn hook
    factories, so the same behaviour now attaches through
    :data:`AgentTurnHookFactory`.

    Two things live here:

    * **LLM telemetry** (MIT-144 / MIT-185 / MIT-202) — Prometheus latency
      histogram plus an ``audit.jsonl`` ``log_llm_call`` record per iteration.
    * **Progress heartbeat** — one evolving ``_status_delta`` message per turn
      so Discord shows "thinking" / "running <tool>" / the latest reasoning
      sentence instead of a silent gap. Published straight onto the bus, which
      decouples it from the old callback plumbing.
    """

    def __init__(self, loop: "AgentLoop", turn: AgentTurnHookContext) -> None:
        super().__init__()
        self._loop = loop
        self._channel = turn.channel
        self._chat_id = turn.chat_id
    ...
    async def _status(self, text: str) -> None:
        ...
        # Keyed by chat_id only (no _stream_id) so the status message survives
        # tool-call segment boundaries and keeps updating in place.
    ...
    async def before_iteration(self, context: AgentHookContext) -> None:
        ...
        await self._status(f"{pick_thinking_emoji()} {label}")
    ...
    async def before_execute_tools(self, context: AgentHookContext) -> None:
        ...
        await self._status(f"{pick_tool_emoji()} {summary}")
    ...
    async def after_iteration(self, context: AgentHookContext) -> None:
        ...
        # Surface the latest complete reasoning sentence, but only while the
        # loop is still calling tools. On a final-content iteration the answer
        # is already streaming into the channel buffer and clobbering it here
        # would replace real response text with a reasoning snippet.
        if response is not None and context.tool_calls:
...
class AgentLoop:
    ...
    def __init__(
        ...
        self._hook_factories: list[AgentTurnHookFactory] = list(hook_factories or [])
        # Ziggy-local (fork): telemetry + Discord heartbeat, per turn.
    ...
    async def _dispatch(self, msg: InboundMessage) -> None:
        ...
        # Ziggy-local (fork): owner-only guard on system-modification requests.
    ...
    async def _process_message(
        ...
        **kwargs: Any,
        ...
        """Ziggy-local (fork, MIT-202): Langfuse root-span wrapper.

        The whole turn runs inside one Langfuse root span so every LLM call,
        tool dispatch and subagent task for this turn shares a single
        ``trace_id``, with ``session_id`` and ``user_id`` propagated to every
        child observation. ``observe_turn`` is a no-op when Langfuse is not
        configured, so upstream behaviour is unchanged without it.
        """
