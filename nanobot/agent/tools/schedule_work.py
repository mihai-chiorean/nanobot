...
_PARAMETERS = tool_parameters_schema(
    ...
    every_seconds=IntegerSchema(
        0,
        description="Interval in seconds when schedule_kind='every'.",
        minimum=1,
    ),
    cron_expr=StringSchema("Cron expression when schedule_kind='cron', e.g. 0 9 * * *."),
    tz=StringSchema("Optional IANA timezone for cron schedules, e.g. America/Los_Angeles."),
    deliverable=StringSchema(
        "Expected durable output, e.g. markdown_digest, report, checklist, artifact_bundle."
    ),
    tools_needed=ArraySchema(
        StringSchema("Tool or connector likely needed."),
        description="Likely tools/connectors, e.g. web_search, web_fetch, gmail, shell.",
    ),
    risk_level=StringSchema(
        "Risk level for this scheduled work.",
        enum=["low", "medium", "high"],
    ),
    risk_notes=StringSchema(
        "Why this is safe or risky. Mention private data, external actions, destructive actions, "
        "paid APIs, or frequency concerns."
    ),
    confirmed=BooleanSchema(
        description=(
            "Set true only after the user has explicitly confirmed this plan and schedule. "
            "Recurring, risky, externally visible, private-data, or ambiguous work requires confirmation."
        ),
        default=False,
    ),
    required=[
    ...
    description=(
        "Create a first-class scheduled Work plan and a cron job that runs it. "
        "The model should draft the plan, use ask_user when confirmation is required, "
        "then call this tool with confirmed=true."
    ),
)
...
class ScheduleWorkTool(Tool):
    """Create a visible scheduled Work plan backed by cron."""
    ...
    def description(self) -> str:
        return (
            ...
            "Use for recurring digests, monitoring, research, reports, and other autonomous work. "
            "If the work is recurring, risky, ambiguous, externally visible, uses private data, "
            "or may spend money, ask the user for confirmation first."
        )
    ...
    async def execute(
        ...
        **_: Any,
        ...
        schedule, schedule_label, delete_after_run, schedule_error = self._build_schedule(
        ...
        metadata = dict(self._metadata.get() or {})
        scope = str(metadata.get("_scope") or metadata.get("work_scope") or "owner")
        ...
        artifact_body = self._format_plan_markdown(plan)
        self._work_store.add_artifact(
            plan_task_id,
            name="work-plan.md",
            kind="markdown",
            content=artifact_body,
            summary=f"{schedule_label}; {deliverable}",
        )

        channel_meta = dict(metadata)
        channel_meta.update(
            {
                "work_scope": scope,
                "work_title": title,
        ...
        self._work_store.update_status(
            plan_task_id,
            "scheduled",
            result_summary=f"Scheduled: {schedule_label}. Deliverable: {deliverable}.",
        )
        return (
            f"Scheduled Work plan '{title}' as job {job.id}. "
            f"Plan task: {plan_task_id}. Schedule: {schedule_label}."
        )
        ...
        *,
        ...
        if kind == "every":
            if not every_seconds or every_seconds <= 0:
                return None, "", False, "Error: every_seconds is required for schedule_kind='every'."
            return (
        ...
        if kind == "at":
            ...
            return (
                CronSchedule(kind="at", at_ms=int(dt.timestamp() * 1000)),
                f"at {dt.isoformat()}",
                True,
                None,
            )
    ...
    def _requires_confirmation(
        *,
        ...
        if schedule_kind in {"cron", "every"}:
            return True
