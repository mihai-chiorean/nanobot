...
_PARAMETERS = tool_parameters_schema(
    ...
    every_seconds=IntegerSchema(
        description="Interval in seconds when schedule_kind='every'.", minimum=1
    ),
...
class ScheduleWorkTool(Tool):
    ...
    def enabled(cls, ctx: ToolContext) -> bool:
        return ctx.cron_service is not None and ctx.work_store is not None

    @classmethod
    def create(cls, ctx: ToolContext) -> Tool:
        if ctx.cron_service is None or ctx.work_store is None:
            raise RuntimeError("ScheduleWorkTool requires a cron service and a work store")
        return cls(
    ...
    def _request_route() -> tuple[str, str, str, dict[str, Any]]:
        """Read routing from the authoritative per-request snapshot.

        0.3.0 removed the legacy ``set_context`` push protocol; built-ins read
        the ``RequestContext`` ContextVar instead (upstream ``043f0e67``).
        """
        ...
        session_key = ctx.session_key or (f"{channel}:{chat_id}" if channel and chat_id else "")
        return session_key, channel, chat_id, dict(ctx.metadata or {})
    ...
    async def execute(
        ...
        **_: Any,
        ...
        if risk_level not in {"low", "medium", "high"}:
            return ToolResult.error("Error: risk_level must be low, medium, or high.")
        if isinstance(context_confidence, bool) or not isinstance(context_confidence, int):
            return ToolResult.error("Error: context_confidence must be an integer from 0 to 100.")
        ...
        session_key, channel, chat_id, request_metadata = self._request_route()
        if not channel or not chat_id:
            return ToolResult.error("Error: no session context (channel/chat_id)")
        plan = {
            "title": title,
            "goal": goal,
        ...
        try:
            ...
            return ToolResult.error("Error: failed to create the scheduled Work job.")
        await self._work_store.run_io(
            self._work_store.append_event,
            plan_task_id,
            "schedule.created",
    ...
    def _build_schedule(
        ...
        *,
        ...
        return None, "", False, ToolResult.error(
            "Error: schedule_kind must be at, every, or cron."
        )
    ...
    def _validate_timezone(timezone_name: str) -> str | None:
        ...
        try:
            ...
            return ToolResult.error(f"Error: unknown timezone '{timezone_name}'")
        return None
