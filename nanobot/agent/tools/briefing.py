...
Ziggy-local (fork, MIT-1028). Ported from the ``feat/shared-rooms`` cutover
lineage onto the 0.3.0 tool contract:

* The turn identity is read from the authoritative request ContextVar
  (:func:`current_request_context`) instead of the legacy positional
  ``set_context(channel, chat_id, ...)`` protocol that 0.3.0 removed.
* Failures return :class:`ToolResult` errors so the runner can classify them.
* Registration is gated by ``enabled()`` on ``tools.briefing.enable``; an
  absent or unmodelled config leaves the tool off (never half-enabled).

Shared-room turns are additionally denied by the room allow-list in
...
class BriefingTool(Tool):
    """Manage the private owner conversation's versioned briefing workflows."""
    ...
    @classmethod
    def config_cls(cls) -> type[BriefingToolsConfig]:
        return BriefingToolsConfig

    @classmethod
    def enabled(cls, ctx: ToolContext) -> bool:
        # An absent or unmodelled briefing section reads as disabled: the
        # registration decision is explicit, never a side effect of a missing
        # key that the model would still see offered for half of a turn.
        return bool(getattr(getattr(ctx.config, "briefing", None), "enable", False))

    @classmethod
    def create(cls, ctx: ToolContext) -> Tool:
        config = getattr(ctx.config, "briefing", None)
        if config is None or not config.enable:
            raise RuntimeError("BriefingTool requires tools.briefing.enable")
        if ctx.sessions is None:
            raise RuntimeError("BriefingTool requires the session manager")
        return cls(config, ctx.sessions)
    ...
    def _turn_identity(self) -> tuple[str, str] | None:
        """Return ``(chat_id, durable origin)`` for a private owner turn.
    ...
    async def execute(
        ...
        if turn is None:
            return ToolResult.error(
                "Error: Briefings can only be managed in a private owner conversation. "
                "Room participants must propose connected work."
            )
        chat_id, origin = turn
        if action != "inspect" and not origin:
            return ToolResult.error(
                "Error: A durable message identity is required. Use the briefing editor."
            )
        try:
            ...
            async with httpx.AsyncClient(
                ...
                if capabilities.get("editorial") is not True or capabilities.get("version") != 1:
                    return ToolResult.error(
                        "Error: Versioned briefings are not enabled on this server."
                    )
                context = await request("GET", "/context", params={"session_key": chat_id})
                workflows: list[dict[str, Any]] = context.get("workflows", [])
                ...
                workflow: dict[str, Any] | None = next(
                    (w for w in workflows if w.get("id") == workflow_id), None
                )
                if action != "create":
                    # Every non-create action mutates an existing workflow.
                    if workflow is None:
                        raise ValueError(
                ...
                if action in {"create", "update", "pause", "resume"}:
                    if action == "create":
                        ...
                        revision: dict[str, Any] = workflow["revisions"][-1]
                        payload = {
                            "session_key": chat_id,
                ...
                assert workflow is not None
                editions: list[dict[str, Any]] = workflow.get("editions", [])
                edition_id = changes.get("edition_id")
                if not edition_id or not any(
                    e["id"] == edition_id for e in editions
                ):
                ...
                if action == "regenerate":
                    workflow = await request(
                        "POST", path + "/run", {"edition_id": edition_id}, key
                    )
                else:
                    ...
                    workflow = await request(
                        "POST",
                        path + "/feedback",
                        {"edition_id": edition_id, "text": comment},
                        key,
                    )
                return json.dumps(
                    {
            ...
            return ToolResult.error("Error: " + str(error))
        except (OSError, httpx.HTTPError, KeyError, TypeError):
            # No provider responses, tokens or private identifiers in diagnostics.
            return ToolResult.error(
                "Error: The briefing operation was not confirmed. Inspect it before retrying. "
                "A saved workflow or queued edition may already exist."
            )
