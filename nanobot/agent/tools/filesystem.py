    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Ziggy-local (fork, MIT-121): sensitive-path blocklist. Refuse to read
            # private keys, credential caches, keyrings, etc. Runs on the raw input
            # AND on the resolved path so a symlink at an innocent name or a ".."
            # traversal that terminates in a sensitive location is still caught.
            #
            # Policy: "better false positive than leak." is_sensitive_path() blocks
            # broadly and deliberately: private key material, credential caches
            # (~/.aws, ~/.config/gcloud, ~/.docker/config.json), .env files, and
            # credential JSON. MIT-140 narrowed the id_*.pub case.
...
class EditFileTool(_FsTool):
    ...
    async def execute(
        ...
        try:
            ...
            # Ziggy-local (fork, MIT-121): sensitive-path blocklist on the edit path.
            # Same "better false positive than leak" policy as read_file.
            if is_sensitive_path(path):
