        ...
        **kwargs: Any,
        ...
        if path:
            ...
            if workspace is None:
                return ToolResult.error("Error: no active workspace.")
            ...
            try:
                ...
                return ToolResult.error(f"Error: artifact path must be inside the workspace: {path}")
            if not candidate.is_file():
                return ToolResult.error(f"Error: artifact path not found: {path}")
            source_path = candidate
        if source_path is None and content is None:
            return ToolResult.error("Error: provide either content or path.")
