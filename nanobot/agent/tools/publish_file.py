...
Ported from the 0.2.x ``feat/shared-rooms`` lineage (MIT-1030).
...
turn an arbitrary file id into a download grant).  Without a bound turn the
tool is inert: the binding is server-owned state that a model turn can never
forge, and it is released on every exit path of the run (see
...
class PublishFileTurn:
    ...
    ``session_manager`` is optional so a turn can still be constructed by
    callers whose :class:`ToolContext` carries no session store; the tool then
    falls back to the manager it was registered with, and publication is
...
def bind_publish_file_turn(turn: PublishFileTurn) -> contextvars.Token[PublishFileTurn | None]:
...
def reset_publish_file_turn(token: contextvars.Token[PublishFileTurn | None]) -> None:
    """Undo one :func:`bind_publish_file_turn` taken by the same task."""
    _current_turn.reset(token)


def current_publish_file_turn() -> PublishFileTurn | None:
    """Return the turn bound for the current agent-run task, if any."""
...
def _read_workspace_markdown(workspace: Path, user_path: object) -> tuple[str, bytes]:
    ...
    ``user_path`` is typed ``object`` on purpose: the schema validates the
    common case, but this runs against whatever the caller passed, and the
    guard below is the last line before descriptor work.
    """
...
class PublishFileTool(Tool):
    ...
    def __init__(self, workspace: Path | str, session_manager: SessionManager | None = None):
    ...
    async def execute(self, path: str = "", **kwargs: Any) -> str:
        ...
        session_manager = turn.session_manager or self._session_manager
        if session_manager is None:
            return "Error: Cannot publish file: session store is unavailable."
        try:
            filename, payload = _read_workspace_markdown(self._workspace, path)
            file_id = session_manager.store_published_snapshot(filename, payload)
        except ValueError as exc:
            return f"Error: Cannot publish file: {exc}"
        except OSError:
            return "Error: Cannot publish file safely."
        turn.remember(file_id, filename)
        url = session_manager.published_file_url(turn.session_key, file_id)
        return (
