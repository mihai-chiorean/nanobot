...
_CRON_PARAMETERS = tool_parameters_schema(
    ...
    as_work=BooleanSchema(
        description=(
            ...
            "instead of a chat reminder. Use for recurring research, monitoring, "
            "digest, or report-generation jobs."
        ),
        default=False,
    ),
    work_title=StringSchema(
        "Optional Work task title when as_work=true. Defaults to the job name."
    ),
...
class CronTool(Tool):
    ...
    def _add_job(
        ...
        if as_work:
            channel_meta.setdefault("work_scope", channel_meta.get("_scope") or "owner")
