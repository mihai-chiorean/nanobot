...
# Ziggy-local (fork, MIT-123): shared secret-dump denylist prescreen.
...
class ExecToolConfig(Base):
    ...
    enable: bool = True
    # Ziggy-local (fork, MIT-203): raised from upstream 60s. Covers claude CLI
    # subagent calls and most go test / pytest runs. 600s stays the hard cap
    # for model-supplied per-call timeouts (ExecTool._MAX_TIMEOUT).
    timeout: int = Field(default=180, ge=0)  # Hard timeout (s); 0 = no limit. Not capped by the per-call max.
    # Ziggy-local (fork, MIT-203): when True, 127.0.0.0/8 and ::1 stop being
    # treated as internal for SSRF purposes. Cloud-metadata (169.254.169.254),
    # RFC1918 and CGNAT stay blocked. Off by default.
...
@tool_parameters(
    tool_parameters_schema(
        ...
        timeout=IntegerSchema(
            description="Hard timeout in seconds (default 180, max 600).",
            minimum=1,
            maximum=600,
        ),
        shell=StringSchema(
...
class ExecTool(Tool):
    ...
    def create(cls, ctx: ToolContext) -> Tool:
        ...
        return cls(
            ...
            # Ziggy-local (fork, MIT-203)
            allow_loopback=getattr(cfg, "allow_loopback", None),
        )
    ...
    def __init__(
        ...
        # Ziggy-local (fork, MIT-203): hard per-instance loopback override.
        # None defers to upstream's WebUI full-access scope check.
    ...
    def _guard_command(
        ...
        *,
        ...
        if contains_internal_url(
            ...
            allow_loopback=bool(self.allow_loopback)
        ...
        # Ziggy-local (fork, MIT-123): secret-dump / key-exfiltration pre-screen.
        # Delegates to the shared denylist in nanobot.utils.sensitive so the same
        # rules apply to shell as to the filesystem tools. Runs last so an
        # out-of-workspace path keeps its own, more specific message.
        if check_shell_command(cmd) is not None:
            return ToolResult.error("Error: Command blocked by safety guard (sensitive data access detected)")
