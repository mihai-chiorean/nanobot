"""NotebookEditTool — cell-level editing of Jupyter .ipynb notebooks."""
...
@tool_parameters(
    tool_parameters_schema(
        ...
        cell_index=IntegerSchema(description="0-based index of the cell to edit", minimum=0),
...
class NotebookEditTool(_FsTool):
    ...
    # ``core`` only: the 0.2.x tool was registered on the main agent loop and
    # never on a subagent, and the cutover review (MIT-1031) found no evidence
    # of use beyond the main loop. Do not widen to ``subagent``/``memory``
    # without demand evidence -- and note it is deliberately NOT in
    # :data:`nanobot.agent.tools.room_policy.ROOM_ALLOWED_TOOLS`.
    _scopes = {"core"}
    ...
    def description(self) -> str:
        return (
            "Edit a Jupyter notebook (.ipynb) at the cell level: replace a "
            "cell's source, insert a new cell after a position, or delete a "
            "cell. Prefer this over edit_file for notebook work -- it operates "
            "on cells (replace/insert/delete by index) instead of forcing you "
            "to match raw JSON text, so cell bookkeeping (ids, outputs, "
            "execution_count) stays correct. Modes: replace (default) "
            "replaces cell content, insert adds a new cell after the target "
            "index, delete removes the cell at the index. cell_index is "
            "0-based. Use edit_file only for small, surgical edits to "
            "non-notebook files or to notebook fields outside cell sources "
            "(e.g. metadata); use this tool for cell-level edits."
        )
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Single shared resolver (MIT-121): raises SensitivePathError, a
            # PermissionError caught below, if the resolved path is credential
            # material. Mirrors write_file/edit_file, which resolve once via
            # _resolve_write rather than handing a raw model string to open().
            fp = self._resolve_write(path)

            # Create a new notebook when the file is absent and mode is insert.
            if not fp.exists():
                if edit_mode != "insert":
            ...
            try:
                ...
                return ToolResult.error(f"Error: Failed to parse notebook: {e}")
