...
class AgentRunner:
    ...
    async def _run_core(
        ...
        for iteration in range(spec.max_iterations):
            # Ziggy-local (fork, MIT-202): one Langfuse span per agent iteration,
            # spanning the provider call AND tool dispatch, so the trace tree reads
            # "turn -> llm-iteration -> generation / tool:<name>". No-op without
            # Langfuse. MIT-210: application exceptions propagate through the span
            # so failed turns are not silently recorded as successful.
            with observe_llm_iteration(iteration=iteration, model=spec.runtime.model):
                ...
                # Ziggy-local (fork, MIT-144/MIT-185): wall-clock latency of the
                # provider call, surfaced to hooks via AgentHookContext.latency_ms.
                ...
                context.latency_ms = (time.monotonic() - _t0) * 1000
