...
if TYPE_CHECKING:
    ...
    from nanobot.agent.tools.recall import MemoryToolConfig
...
class ToolsConfig(Base):
    ...
    # 0.3.0 deleted `tools.rag` and let the RAG tools auto-register from a bare
    # `import chromadb`, which left tenant generators setting a key that no
    # longer existed. This one exists.
    memory: MemoryToolConfig = Field(
        default_factory=lambda: _lazy_default("nanobot.agent.tools.recall", "MemoryToolConfig"),
    )
