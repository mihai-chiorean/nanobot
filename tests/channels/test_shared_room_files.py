...
# ---------------------------------------------------------------------------
# The shareable projection's session gate -- the helper ``_shareable_room_messages``
# consumes is the same one ``/_history`` ``resolve_read_func`` uses.  A raw
# uid must be wrapped as ``session_key`` and compared to the credential's own
# session; without a room session the helper fails closed -- which is how a
# non-room (e.g. owner-private) conversation must be refused rather than
# fetched local and compared.
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_room_share_helper_rejects_missing_or_mismatched_session(
    sessions: SessionManager, store: SharedRoomStore
) -> None:
    # The gate is credential-bound, like the room route's own ``_history``
    # ``resolve_read_func``: it authorizes a presented (session, user) pair
    # against the room under ``session.user_name``'s view -- so a bad
    # ``message_id`` skips the entry, and a whole conversation belonging to
    # another principal must be refused outright, never fetched locally and
    # compared.  Here: the helper may not fall back to an arbitrary store
    # session when the caller's own session is absent.
    token, credential = store.mint(
        room_id=ROOM_ID,
        chat_id=ROOM_CHAT,
        participant_id=PARTICIPANT_ID,
        display_name="Guest",
    ...
    assert store.authorizes_session(credential, credential.session_key) is True
    assert store.authorizes_session(credential, "mismatched-session") is False
    # A non-room (un-minted) session key must not resolve through the gate.
    assert store.authorizes_session(credential, "websocket:stranger") is False
    assert store.authorizes_session(credential, "") is False
    # The room's own key, after the room exists, is the credential's own
    # session -- exactly what the route must accept.
    await _create_room(
        SharedRoomRouter(config=_Config(), sessions=sessions, store=store),
        _room_key(OWNER_CHAT),
        ROOM_CHAT,
        ROOM_ID,
    )
    token, credential = store.mint(
    ...
    assert store.authorizes_session(credential, _room_key(ROOM_CHAT)) is True


@pytest.mark.asyncio
async def test_owner_role_check_and_audit_reads(
    sessions: SessionManager, store: SharedRoomStore, router: SharedRoomRouter
) -> None:
    await _create_room(router, _room_key(OWNER_CHAT), ROOM_CHAT, ROOM_ID)
    file_id = await _publish(sessions, _room_key(ROOM_CHAT), content=b"served\n")
    token = _mint(store, ROOM_ID, ROOM_CHAT)
    ok = await _guest_get(router, _target(ROOM_CHAT, file_id), token)
    assert ok is not None and ok.status_code == 200
    # A room-adjacent read that must resolve via the credential's own session.
    messages_target = f"/api/sessions/{_enclosed_key(ROOM_CHAT)}/messages"
    ...
    # The room shares its conversation with the joined participant; the room's
    # own key must resolve through the helper for the room's own session.
    data = sessions.read_session_file(_room_key(ROOM_CHAT))
    assert data is not None
    mine = [m for m in data["messages"] if m.get("role") == "assistant"]
    assert len(mine) >= 1
    for message in mine:
        assert store.authorizes_session(
            store.api_credential(token), _room_key(ROOM_CHAT)
        ) is True


# ---------------------------------------------------------------------------
# Clone coverage: ``clone_session_for_shared_room`` rewrites transcript links
# to the room's own key and copies grants; the copy may only ever be a
# translation of server-recorded state, so it is compared against the source
