...
@pytest.mark.asyncio
@pytest.mark.parametrize("pool_name", ["_issued_tokens", "_api_tokens"])
async def test_token_issue_rejects_when_either_pool_at_capacity(
    bus: MagicMock, pool_name: str
) -> None:
    port = 29881
    ...
    try:
        # Filling either pool must block issuance before a token can be
        # registered in the other pool.
        setattr(
            channel,
            pool_name,
