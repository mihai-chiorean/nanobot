...
| `tools.rag.enable` | `false` | Registers the Chroma-backed `recall` and `ingest` tools. RAG data is stored in the active workspace and ingest is restricted to that workspace. Keep disabled unless semantic document recall is intentionally provisioned for that instance. |
